...
-- =========================================================================
-- 8. Hide Student from Leaderboard Column & Updated Leaderboard RPC
-- =========================================================================
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'users' AND column_name = 'is_hidden_from_leaderboard'
    ) THEN
        ALTER TABLE public.users ADD COLUMN is_hidden_from_leaderboard BOOLEAN DEFAULT FALSE;
    END IF;
END $$;

-- Update Leaderboard RPC to exclude hidden students
...
BEGIN
    ...
    WITH student_metrics AS (
        ...
        WHERE u.role = 'student' 
          AND u.status = 'active'
          AND COALESCE(u.is_hidden_from_leaderboard, false) = false
        GROUP BY u.id
    )
    SELECT 
        s_id as user_id,
